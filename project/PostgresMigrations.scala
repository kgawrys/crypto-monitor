...
object PostgresMigrations {
  ...
  def dbName     = env.getOrElse("POSTGRES_DB", "cryptomonitor")
  def testDbName = env.getOrElse("POSTGRES_TEST_DB", s"$dbName-test")
  def dbServer   = env.getOrElse("POSTGRES_SERVICE_HOST", "localhost")
